<?php
/**
 * صفحه وبلاگ و اخبار (نسخه ماژولار) - بازنویسی شده برای ساختار جدید دیتابیس
 * 
...
$sql = "SELECT p.post_id, p.category_id, p.published_at, p.views, 
               ...
               c.category_id, ct.name as category_name, m.filepath as image_path
        FROM cms_posts p
...
$popular_sql = "SELECT p.post_id, p.views, pt.title, pt.content,
                       m.filepath as image_path, c.category_id, ct.name as category_name
                FROM cms_posts p
                INNER JOIN cms_post_translations pt ON p.post_id = pt.post_id AND pt.language_id = (
                    SELECT language_id FROM core_languages WHERE code = ?
                )
