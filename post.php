<?php
/**
 * صفحه نمایش پست تکی با قابلیت‌های پیشرفته و محتوای ماژولار - بازنویسی شده برای ساختار جدید دیتابیس
 *
...
function getPostContent($key, $lang, $conn) {
    // اول بررسی می‌کنیم آیا در جدول temp_post_content و temp_post_translations موجود است
    $sql = "SELECT tpt.content_value 
            FROM temp_post_content tpc
            JOIN temp_post_translations tpt ON tpc.content_id = tpt.content_id
            JOIN core_languages l ON tpt.language_id = l.code
            WHERE tpc.field_key = ? 
            AND l.code = ? 
            AND tpc.is_active = 1 
            LIMIT 1";
    ...
    // اگر در جدول temp_post_content و temp_post_translations پیدا نشد، از جدول temp_blog_content و temp_blog_translations استفاده می‌کنیم
    $sql = "SELECT bt.content_value 
            FROM temp_blog_content bc
            JOIN temp_blog_translations bt ON bc.content_id = bt.content_id
            JOIN core_languages l ON bt.language_id = l.code
            WHERE bc.field_key = ? 
            AND l.code = ? 
            AND bc.is_active = 1 
            LIMIT 1";
...
// افزایش تعداد بازدید
$updateViewsSql = "UPDATE cms_posts SET views = views + 1 WHERE post_id = ?";
...
$sql = "SELECT p.post_id, p.category_id, p.published_at, p.views, 
               ...
               m.filepath as main_image
        FROM cms_posts p
        INNER JOIN cms_post_translations pt ON p.post_id = pt.post_id AND pt.language_id = (
            SELECT language_id FROM core_languages WHERE code = ?
        )
        LEFT JOIN cms_categories c ON p.category_id = c.category_id
        LEFT JOIN cms_category_translations ct ON c.category_id = ct.category_id AND ct.language_id = (
            SELECT language_id FROM core_languages WHERE code = ?
        )
...
$stmt->bind_param("ssi", $lang, $lang, $postId);
$stmt->execute();
$result = $stmt->get_result();
...
$categories_sql = "SELECT c.category_id, ct.name as category_name, COUNT(p.post_id) as post_count
                   ...
                   LEFT JOIN cms_category_translations ct ON c.category_id = ct.category_id
                   LEFT JOIN cms_posts p ON c.category_id = p.category_id AND p.status = 'published'
                   WHERE ct.language_id = (SELECT language_id FROM core_languages WHERE code = ?)
...
$related_sql = "SELECT p.post_id, pt.title, m.filepath as main_image, p.published_at
                FROM cms_posts p
                INNER JOIN cms_post_translations pt ON p.post_id = pt.post_id
                LEFT JOIN media_files m ON p.main_media_id = m.id
                WHERE p.category_id = ? 
                AND p.post_id != ? 
                AND pt.language_id = (SELECT language_id FROM core_languages WHERE code = ?)
...
$related_posts = [];
if ($related_stmt) {
    $related_stmt->bind_param("iis", $post['category_id'], $postId, $lang);
...
// گرفتن آخرین پست‌ها
$latest_sql = "SELECT p.post_id, pt.title, m.filepath as main_image, p.published_at
               FROM cms_posts p
               INNER JOIN cms_post_translations pt ON p.post_id = pt.post_id
               LEFT JOIN media_files m ON p.main_media_id = m.id
               WHERE p.post_id != ? 
...
$latest_posts = [];
if ($latest_stmt) {
    $latest_stmt->bind_param("is", $postId, $lang);
    $latest_stmt->execute();
