...
class ContentManager {
    ...
    /**
     * مقداردهی اولیه کش محتوا - بازنویسی شده برای ساختار جدید
     * 
     * @return void
     */
    private function initializeCache() {
        ...
        // بارگیری همه محتوای ثابت برای زبان فعلی از جداول جدید
        $query = "SELECT c.field_key, t.content_value as content 
                  FROM temp_register_content c
                  JOIN temp_register_translations t ON c.content_id = t.content_id
                  WHERE t.language_id = ? AND c.is_repeatable = 0";
    ...
    /**
     * دریافت محتوای متنی با کلید مشخص - بازنویسی شده برای ساختار جدید
     * 
    ...
    public function getContent($key, $default = '') {
        ...
        $query = "SELECT t.content_value as content 
                  FROM temp_register_content c
                  JOIN temp_register_translations t ON c.content_id = t.content_id
                  WHERE c.field_key = ? AND t.language_id = ? AND c.is_repeatable = 0 
                  LIMIT 1";
    ...
    /**
     * دریافت محتوای تکرارشونده با سکشن مشخص - بازنویسی شده برای ساختار جدید
     * 
    ...
    public function getRepeatableContent($sectionId) {
        ...
        $query = "SELECT c.field_key, t.content_value as content 
                  FROM temp_register_content c
                  JOIN temp_register_translations t ON c.content_id = t.content_id
                  WHERE c.section_id = ? AND t.language_id = ? AND c.is_repeatable = 1 
...
class FormManager {
    ...
    /**
     * دریافت شهرهای مربوط به سرویس حمل و نقل - بازنویسی شده برای ساختار جدید
     * 
    ...
    public function getTransportationCities() {
        ...
        $placeholders = str_repeat('?,', count($cityKeys) - 1) . '?';
        $query = "SELECT c.field_key, t.content_value as content 
                  FROM temp_register_content c
                  JOIN temp_register_translations t ON c.content_id = t.content_id
                  WHERE c.field_key IN ({$placeholders}) 
                  AND t.language_id = ?";
        ...
        $bindParams = array_merge($cityKeys, [$langCode]);
    ...
    class StorageManager {
        ...
        private function saveTransportation($studentId, $data) {
            if (!empty($data['transportationCity']) && !empty($data['transportationRoute'])) {
                ...
                $checkQuery = "SELECT route_id FROM service_bus_routes WHERE route_id = ?";
...
/**
 * دریافت محتوای صفحه ثبت‌نام - بازنویسی شده برای ساختار جدید
 * 
 * @param string $key کلید محتوا
...
function getTransportationCities($lang) {
    ...
    $bindParams = array_merge($cityKeys, [$lang]);
    $types = str_repeat('s', count($bindParams));
