<?php
/**
 * توابع کمکی برای سیستم وبلاگ ماژولار - بازنویسی شده برای ساختار جدید دیتابیس
 * 
...
/**
 * دریافت محتوای ثابت از جدول temp_blog_content
 * 
...
function getBlogContent($field_key, $lang = null) {
    ...
    // ابتدا از محتوای ترجمه شده در جدول temp_blog_translations استفاده می‌کنیم
    $query = "SELECT bt.content_value 
              ...
              WHERE bc.field_key = '{$field_key}' 
              AND l.code = '{$lang}' 
              AND bc.is_repeatable = 0 
              AND bc.is_active = 1
              LIMIT 1";
    ...
    // اگر ترجمه پیدا نشد، از محتوای اصلی استفاده می‌کنیم
    $query = "SELECT bc.content
              FROM temp_blog_content bc
              WHERE bc.field_key = '{$field_key}' 
              AND bc.is_repeatable = 0 
              AND bc.is_active = 1
...
function isWidgetVisible($widget_key) {
    ...
    $query = "SELECT content_id FROM temp_blog_content 
...
function getSidebarWidgetsOrder() {
    ...
    $query = "SELECT field_key, content FROM temp_blog_content 
