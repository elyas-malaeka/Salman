...
class FormBuilder {
    ...
    private function renderFileField($field) {
        ...
        $output .= '<div class="file-upload-text">' . $this->getTranslation('file_type_hint', 'JPEG/PNG/PDF, max size: 5MB') . '</div>';
        $output .= '<div class="file-upload-hint">' . $this->getTranslation('file_upload_placeholder', 'Click to upload or drag and drop a file here') . '</div>';
    ...
    public function getTransportationCities() {
        ...
        try {
            // استفاده از جدول جدید با ساختار متفاوت
            $query = "SELECT c.field_key, t.content_value as content 
                      FROM temp_register_content c
                      JOIN temp_register_translations t ON c.content_id = t.content_id
                      WHERE c.field_key IN ('dubai', 'sharjah', 'ajman') 
                      AND t.language_id = ?";
