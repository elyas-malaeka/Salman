...
$language_id = $current_lang['code']; // Changed to use language code instead of ID
 ...
 * @param string $fallback_lang Fallback language code if content not found
 * @return string Content or empty string if not found
 */
function getDbContent($field_key, $section_id = null, $fallback_lang = 'en') {
    global $db, $language_id;
    
    // Build the query - get content from temp_home_translations via temp_home_content
    $query = "SELECT t.content_value 
              FROM temp_home_content c
              JOIN temp_home_translations t ON c.content_id = t.content_id 
              WHERE c.field_key = ? 
              AND t.language_id = ?";
    ...
    if ($language_id != $fallback_lang) {
        $fallback_query = "SELECT t.content_value 
                           FROM temp_home_content c
                           JOIN temp_home_translations t ON c.content_id = t.content_id 
                           WHERE c.field_key = ?";
        ...
        if ($section_id) {
            $fallback_query .= " AND c.section_id = ?";
            $fallback_params[] = $section_id;
            $fallback_types .= "s";
        }
        
        $fallback_query .= " AND t.language_id = ? LIMIT 1";
        $fallback_params[] = $fallback_lang;
        $fallback_types .= "s";
 ...
 */
function getRepeatableDbContent($field_key, $section_id, $fallback_lang = 'en') {
    global $db, $language_id;
    
    // Query to get repeatable content for current language
    $query = "SELECT c.content_id, c.field_key, c.section_id, c.sort_order, c.image_path, t.content_value AS content 
              FROM temp_home_content c
              JOIN temp_home_translations t ON c.content_id = t.content_id 
              WHERE c.field_key = ? 
              AND c.section_id = ? 
    ...
    $stmt = $db->prepare($query);
    $stmt->bind_param("sss", $field_key, $section_id, $language_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($language_id != $fallback_lang) {
        $fallback_query = "SELECT c.content_id, c.field_key, c.section_id, c.sort_order, c.image_path, t.content_value AS content 
                          FROM temp_home_content c
                          JOIN temp_home_translations t ON c.content_id = t.content_id 
                          WHERE c.field_key = ? 
                          AND c.section_id = ? 
        ...
        $fallback_stmt = $db->prepare($fallback_query);
        $fallback_stmt->bind_param("sss", $field_key, $section_id, $fallback_lang);
        $fallback_stmt->execute();
        $fallback_result = $fallback_stmt->get_result();
...
<html lang="<?php echo $lang; ?>" dir="<?php echo $isRtl ? 'rtl' : 'ltr'; ?>">
    <head>
        ...
        <!-- Main CSS -->
        <?php include_once 'assets/css/home.css.php'; ?>
